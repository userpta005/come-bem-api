...
class RequisitionController extends Controller
{
    ...
    {
        $this->middleware('permission:requisitions_create', ['only' => ['create', 'store']]);
        $this->middleware('permission:requisitions_edit', ['only' => ['edit', 'update']]);
        $this->middleware('permission:requisitions_view', ['only' => ['show', 'index']]);
        $this->middleware('permission:requisitions_delete', ['only' => ['destroy']]);
    ...
    public function index(Request $request)
    {
        ...
        return view('requisitions.index', compact('data', 'products'));
    }
    ...
    public function create(Valuestore $settings)
    {
        ...
        return view('requisitions.create', compact('stores'));
    }
        ...
        DB::transaction(function () use ($request) {
            for ($i = 0; $i < count($request->stock_id); $i++) {
                ...
                Stock::query()
                    ...
                    ->decrement('quantity', moneyToFloat($request->quantity[$i]));
            }
        });
    ...
    public function show($id)
    {
        ...
        return view('requisitions.show', compact('item'));
    }
